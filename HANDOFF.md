...
- **Option B build spec:** `docs/sure-reference/` holds clean-room reference docs (Budgets · Reports · Dashboard · Balance Sheet) reverse-engineered from Sure (`we-promise/sure`) and rewritten in wealth-os terms — these are **formulas, not Sure source** (Sure is AGPL-3.0). Start at `docs/sure-reference/99_INDEX.md` + `00_PORTING_GUIDE.md`.
