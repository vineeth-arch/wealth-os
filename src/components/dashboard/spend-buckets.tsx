...
const prefixOf = (parent: string) => parent.slice(0, 2);

/**
 * "Where money went" + "Leakage watchlist". Each row links to that Halan parent's dedicated page
 * (/buckets/[prefix]) — leaf categories, totals, and inline-editable transactions for the period —
 * instead of opening a modal. Prefix-based routing is rename-safe (CLAUDE.md).
 */
export function SpendBuckets({ buckets, leak }: { buckets: Bucket[]; leak: Leak[] }) {
    ...
    <div className="grid gap-6 lg:grid-cols-2">
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          {buckets.map((b) => (
            <Link key={b.parent} href={`/buckets/${prefixOf(b.parent)}`}
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          {leak.map((l) => (
            <Link key={l.parent} href={`/buckets/${prefixOf(l.parent)}`}
              className="block w-full space-y-1 rounded-md text-left transition-colors hover:bg-accent/40 focus:outline-none focus:ring-2 focus:ring-ring">
              <div className="flex items-center justify-between text-sm">
